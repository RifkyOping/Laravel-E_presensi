        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-4 py-3 flex-shrink-0">
                ...
                <div>
                    ...
                    <p class="text-[.65rem] text-green-600">Skor suara: {{ $progres->skor_suara ?? '-' }}% | Kuis: {{ $progres->skor_kuis ?? '-' }}</p>
                </div>
            </div>
            @elseif($progres->lulus_suara)
            <div class="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 flex-shrink-0">
            ...
            @else
            <div class="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex-shrink-0">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                ...
                <div class="flex flex-col items-center justify-center py-20 text-center bg-slate-50 px-8">
                    <svg class="w-16 h-16 text-amber-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                              d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                    </svg>
                    <p class="font-bold text-slate-500 text-lg mb-1">File PDF tidak ditemukan</p>
                    <p class="text-sm text-slate-400">Silakan hubungi admin untuk mengunggah ulang file PDF buku ini.</p>
                </div>
                @endif
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-5 space-y-4" id="voicePanel">
                ...
                <p class="text-xs text-slate-500 leading-relaxed">
                    Bacakan isi E-Book untuk diverifikasi. Sistem akan mencocokkannya dengan buku. Minimal <strong>60%</strong> kesamaan untuk lulus.
                </p>

                {{-- Visualizer --}}
                <div id="visualizer"
                     class="h-16 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-center gap-1 px-4 overflow-hidden">
                    <span class="text-xs text-slate-400 font-medium" id="visualizerText">Tekan tombol untuk mulai merekam</span>
                ...
                <div class="flex gap-2">
                    <button id="btnRecord"
                            class="flex-1 flex items-center justify-center gap-2 bg-[#1e3a6e] hover:bg-[#162d57]
                ...
                <button id="btnVerify"
                        class="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-[#1e3a6e] to-[#2d5299]
                ...
                <div class="space-y-2">
                    <a href="{{ route('ebook.index') }}"
                       class="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors group">
                        <svg class="w-4 h-4 text-slate-400 group-hover:text-[#1e3a6e]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                        </svg>
                        <span class="text-sm font-medium text-slate-600 group-hover:text-[#1e3a6e]">Kembali ke Koleksi</span>
                    </a>
                </div>
            </div>
        </div>
