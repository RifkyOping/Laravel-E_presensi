...
<head>
    ...
    <title>{{ $title ?? config('app.name', 'E-Presensi') }} — SMKN 1 Majene</title>
    ...
    <style>
        ...
        * { font-family: 'Inter', sans-serif; }
        ...
        .app-nav:hover { background: #f0f4ff; color: #1e3a6e; }
        .app-nav.active {
            background: linear-gradient(135deg, #1e3a6e, #2d5099);
            color: #fff; box-shadow: 0 4px 14px rgba(30,58,110,.28);
        }
        .app-nav svg { flex-shrink: 0; opacity: .65; }
        .app-nav.active svg, .app-nav:hover svg { opacity: 1; }
        ...
        /* ── Cards & Tables ── */
        .app-card { background: #fff; border-radius: 16px; border: 1px solid #e2e8f0; }
        .app-tbl thead th {
            background: #f8fafc; color: #64748b; font-size: .71rem; font-weight: 700;
            letter-spacing: .07em; text-transform: uppercase;
            padding: .75rem 1rem; border-bottom: 2px solid #e2e8f0;
        }
        .app-tbl tbody tr { border-bottom: 1px solid #f1f5f9; transition: background .15s; }
        .app-tbl tbody tr:hover { background: #f8fafc; }
        .app-tbl tbody td { padding: .78rem 1rem; font-size: .84rem; color: #334155; }

        /* ── Badge ── */
        .app-badge { padding: .22rem .65rem; border-radius: 999px; font-size: .71rem; font-weight: 700; display: inline-block; }
        ...
        .app-input {
            ...
            color: #1e293b; outline: none; transition: border .18s, box-shadow .18s; background: #fff;
        }
        .app-input:focus { border-color: #1e3a6e; box-shadow: 0 0 0 3px rgba(30,58,110,.1); }
        .app-label {
            font-size: .72rem; font-weight: 700; color: #64748b;
            text-transform: uppercase; letter-spacing: .07em; display: block; margin-bottom: .3rem;
        }

        /* ── Buttons ── */
        .btn-primary {
            background: #1e3a6e; color: #fff; border: none; padding: .55rem 1.1rem;
        ...
        .btn-outline {
            ...
            padding: .55rem 1rem; border-radius: 10px; font-weight: 600; font-size: .83rem;
        ...
        .stat-card:hover { transform: translateY(-3px); box-shadow: 0 12px 30px rgba(0,0,0,.08); }
        ...
        /* ── Mobile overlay ── */
        #app-overlay { display: none; position: fixed; inset: 0; background: rgba(0,0,0,.45); z-index: 40; }
        #app-overlay.active { display: block; }

        /* ── Animations ── */
        @keyframes fadeUp { from { opacity: 0; transform: translateY(14px); } to { opacity: 1; transform: none; } }
        .anim-up { animation: fadeUp .35s ease forwards; }
    ...
    <aside id="app-sidebar" class="flex flex-col overflow-y-auto overflow-x-hidden h-full shadow-sm flex-shrink-0">
        ...
        <div class="flex items-center gap-3 px-4 py-4 border-b border-slate-100">
            ...
            <div class="min-w-0">
                ...
                @endphp
                <span class="inline-block text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide {{ $roleLabel[1] }}">
                    {{ $roleLabel[0] }}
                </span>
            </div>
        </div>
        ...
        <nav class="flex-1 px-3 py-4">
            ...
            @if(Auth::user()->role === 'admin')
                ...
                <a href="{{ route('admin.dashboard') }}" class="app-nav {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                    Dashboard
                </a>
                <span class="app-section">Manajemen</span>
                <a href="{{ route('admin.users') }}" class="app-nav {{ request()->routeIs('admin.users*') ? 'active' : '' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                    </svg>
                    Pengguna
                </a>
                <a href="{{ route('admin.mata-pelajaran.index') }}" class="app-nav {{ request()->routeIs('admin.mata-pelajaran*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                    Mata Pelajaran
                </a>
                <a href="{{ route('admin.ebook.index') }}" class="app-nav {{ request()->routeIs('admin.ebook*') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                    E-Book Literasi
                </a>
                <span class="app-section">Monitoring</span>
                <a href="{{ route('admin.absensi-guru') }}" class="app-nav {{ request()->routeIs('admin.absensi-guru') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>
                    Absensi Guru
                </a>
                <a href="{{ route('admin.absensi-siswa') }}" class="app-nav {{ request()->routeIs('admin.absensi-siswa') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                    Absensi Siswa
                </a>
                <a href="{{ route('admin.aktivitas-guru') }}" class="app-nav {{ request()->routeIs('admin.aktivitas-guru') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    Aktivitas Mengajar
                </a>
                <span class="app-section">Pengaturan</span>
                <a href="{{ route('admin.geofence') }}" class="app-nav {{ request()->routeIs('admin.geofence') ? 'active' : '' }}">
            ...
            @elseif(Auth::user()->role === 'guru')
                <span class="app-section">Menu Utama</span>
                <a href="{{ route('guru.dashboard') }}" class="app-nav {{ request()->routeIs('guru.dashboard') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                    Dashboard
                </a>
                <span class="app-section">Presensi</span>
                <a href="{{ route('guru.absensi') }}" class="app-nav {{ request()->routeIs('guru.absensi') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>
                    Absen Sekolah
                </a>
                <a href="{{ route('guru.aktivitas') }}" class="app-nav {{ request()->routeIs('guru.aktivitas') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    Aktivitas Mengajar
                </a>
                <a href="{{ route('guru.literasi.quran') }}" class="app-nav {{ request()->routeIs('guru.literasi.quran') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    Literasi Al Quran
                </a>
            ...
            @elseif(Auth::user()->role === 'siswa')
                <span class="app-section">Menu Utama</span>
                <a href="{{ route('siswa.dashboard') }}" class="app-nav {{ request()->routeIs('siswa.dashboard') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                    Dashboard
                </a>
                ...
                <a href="{{ route('siswa.quran') }}" class="app-nav {{ request()->routeIs('siswa.quran') ? 'active' : '' }}">
            ...
            @elseif(Auth::user()->role === 'pengawas')
                <span class="app-section">Menu Utama</span>
                <a href="{{ route('pengawas.dashboard') }}" class="app-nav {{ request()->routeIs('pengawas.dashboard') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                    Dashboard
                </a>
            ...
            @elseif(Auth::user()->role === 'kurikulum')
                <span class="app-section">Menu Utama</span>
                <a href="{{ route('kurikulum.dashboard') }}" class="app-nav {{ request()->routeIs('kurikulum.dashboard') ? 'active' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                    Dashboard
                </a>
                <span class="app-section">Verifikasi Mengajar</span>
                <a href="{{ route('kurikulum.monitoring-mengajar') }}" class="app-nav {{ request()->routeIs('kurikulum.monitoring-mengajar') ? 'active' : '' }}">
        ...
        <div class="px-3 py-4 border-t border-slate-100 space-y-1">
            <a href="{{ route('profile.edit') }}" class="app-nav {{ request()->routeIs('profile.edit') ? 'active' : '' }}">
