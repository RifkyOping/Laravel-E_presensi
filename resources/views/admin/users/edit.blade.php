...
<div class="max-w-3xl space-y-6">
    ...
    <form method="POST" action="{{ route('admin.users.update', $user->id) }}" class="space-y-5">
            ...
            <div class="px-6 py-6 space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="nis" value="{{ old('nis', $user->siswaProfile?->nis) }}" placeholder="Nomor Induk Siswa"
                ...
                <div>
                    ...
                    <select name="agama"
                        ...
                        <option value="{{ $ag }}" {{ strtolower(old('agama', $user->siswaProfile?->agama)) === strtolower($ag) ? 'selected' : '' }}>{{ $ag }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        {{-- Jabatan Sekolah --}}
        <div id="profilJabatan" class="{{ in_array(old('role', $user->role), ['guru']) ? '' : 'hidden' }}
            ...
            <div class="px-6 py-6 space-y-4">
                <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-100 hover:border-indigo-200 bg-slate-50 hover:bg-indigo-50/30 cursor-pointer transition group">
                    <input type="checkbox" name="is_kepsek" value="1"
                           {{ old('is_kepsek', $user->guruProfile?->is_kepsek) ? 'checked' : '' }}
                ...
                <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-100 hover:border-indigo-200 bg-slate-50 hover:bg-indigo-50/30 cursor-pointer transition group">
                    <input type="checkbox" name="is_kurikulum" value="1"
                           {{ old('is_kurikulum', $user->guruProfile?->is_kurikulum) ? 'checked' : '' }}
                           class="w-5 h-5 text-indigo-600 border-slate-300 rounded focus:ring-indigo-600">
                    <div class="flex flex-col">
