        ...
        <form method="POST" action="{{ route('admin.mata-pelajaran.update', $mataPelajaran->id) }}" class="px-6 py-6 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="tingkat"
                        ...
                        <option value="{{ $t }}" {{ old('tingkat',$mataPelajaran->tingkat)===$t?'selected':'' }}>
                ...
                <div>
                    ...
                    <input type="text" name="jurusan" value="{{ old('jurusan', $mataPelajaran->jurusan) }}" placeholder="Contoh: TKJ, RPL, AK"
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3" placeholder="Deskripsi singkat tentang mata pelajaran ini..."
                          class="w-full border border-slate-200 focus:border-[#1e3a6e] focus:ring-2 focus:ring-[#1e3a6e]/10 rounded-xl px-4 py-2.5 text-slate-800 font-medium focus:outline-none transition text-sm resize-none">{{ old('deskripsi', $mataPelajaran->deskripsi) }}</textarea>
            </div>
