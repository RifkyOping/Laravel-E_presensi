    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div>
            ...
            <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="username"
                   class="block w-full rounded-xl border-2 {{ $errors->has('email') ? 'border-red-400 bg-red-50 focus:border-red-500' : 'border-[#24417c]/20 focus:border-[#24417c]' }} focus:ring-0 text-[#24417c] shadow-sm transition duration-300">
            @if ($errors->has('email'))
        ...
        <div class="flex items-center justify-between pt-2">
            ...
            @if (Route::has('password.request'))
                <a class="text-sm font-bold text-[#24417c]/70 hover:text-[#24417c] underline underline-offset-4 transition duration-300" href="{{ route('password.request') }}">
                    {{ __('Lupa Kata Sandi?') }}
                </a>
            @endif
        </div>
        ...
        <!-- Tombol Login -->
        <div class="pt-6">
            <button type="submit" class="w-full flex justify-center items-center bg-gradient-to-r from-[#24417c] to-blue-600 text-white font-bold text-lg px-6 py-3.5 rounded-xl border border-blue-400 hover:shadow-lg hover:shadow-blue-500/30 transition-all duration-300 hover:-translate-y-1 relative overflow-hidden group">
