<x-app-layout>
    ...
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <a href="{{ route('piket.indikator.create') }}"
            ...
            <div class="block sm:hidden divide-y divide-slate-100">
                ...
                <div class="p-4 hover:bg-slate-50/50 transition">
                    <div class="flex items-start justify-between gap-3 mb-3">
                        <div class="flex-1">
                            <span class="text-xs font-black text-slate-400 mb-1 block">#{{ $index + 1 }}</span>
                            <p class="font-bold text-slate-800 text-sm leading-snug">{{ $indikator->pertanyaan }}</p>
                        </div>
                        <div class="flex-shrink-0 mt-5">
                            @if($indikator->aktif)
                                <span class="inline-flex px-2 py-1 text-[10px] font-bold rounded-md bg-green-50 text-green-700 border border-green-200 leading-none">
                                    Aktif
                                </span>
                            @else
                                <span class="inline-flex px-2 py-1 text-[10px] font-bold rounded-md bg-slate-100 text-slate-500 border border-slate-200 leading-none">
                                    Nonaktif
                                </span>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center gap-2">
                        <a href="{{ route('piket.indikator.edit', $indikator->id) }}"
                           class="flex-1 flex justify-center items-center gap-1.5 px-3 py-2 rounded-lg border border-[#1e3a6e]/30 text-[#1e3a6e] hover:bg-[#1e3a6e]/5 font-bold text-xs transition duration-200">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            Edit
                        </a>
                        <form method="POST" action="{{ route('piket.indikator.destroy', $indikator->id) }}"
                              onsubmit="return confirm('Hapus pertanyaan ini?')" class="flex-1">
                            @csrf @method('DELETE')
                            <button type="submit"
                                    class="w-full flex justify-center items-center gap-1.5 px-3 py-2 rounded-lg border border-red-200 text-red-600 hover:bg-red-50 font-bold text-xs transition duration-200">
