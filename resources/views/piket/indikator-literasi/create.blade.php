<x-app-layout>
    <x-slot name="header">
        <span class="text-sm font-bold text-slate-800">Tambah Pertanyaan Indikator Literasi</span>
    </x-slot>

    <div class="max-w-2xl space-y-6">
        <div class="flex items-center gap-3">
            <a href="{{ route('piket.indikator.index') }}" class="text-slate-400 hover:text-slate-700 transition">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            </a>
            <h2 class="text-xl font-black text-slate-800">Tambah Pertanyaan Indikator Literasi</h2>
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
            ...
            <form action="{{ route('piket.indikator.store') }}" method="POST" class="px-6 py-6 space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="pertanyaan" id="pertanyaan" rows="3" required
                              class="w-full border border-slate-200 focus:border-[#1e3a6e] focus:ring-2 focus:ring-[#1e3a6e]/10 rounded-xl px-4 py-2.5 text-slate-800 font-medium focus:outline-none transition text-sm @error('pertanyaan') border-red-300 @enderror">{{ old('pertanyaan') }}</textarea>
                    @error('pertanyaan')
