...
<div class="space-y-6">
    ...
    @endphp
    <div x-data="{ 
        showFilter: localStorage.getItem('filter_pengawas_absensi_siswa') === 'true' || {{ $hasFilter ? 'true' : 'false' }} 
    }" 
    x-init="$watch('showFilter', val => localStorage.setItem('filter_pengawas_absensi_siswa', val))"
    class="app-card p-6">
        ...
        <div class="block sm:hidden divide-y divide-slate-100">
            ...
            <div class="px-4 py-3 flex items-center gap-3">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-slate-400">{{ Carbon::parse($r->tanggal)->translatedFormat('d M Y') }}</p>
            ...
            @empty
            <div class="py-8 text-center text-slate-400 text-sm">Belum ada riwayat absensi siswa.</div>
            @endforelse
        </div>
                    ...
                    <tr>
                        ...
                        <td class="text-left">
                            <div class="flex items-center gap-2.5">
                                <div class="w-8 h-8 rounded-full bg-[#1e3a6e] text-white flex items-center justify-center font-bold text-xs flex-shrink-0">{{ strtoupper(substr($r->user->name, 0, 1)) }}</div>
